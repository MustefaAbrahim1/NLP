...
class InformationRetrieval():
    ...
    def buildIndex(self, docs, docIDs):
        ...
        #merging the sentences in each doc to get one big list for each doc
        merged_docs = []
        ...
        model = gensim.models.Word2Vec(merged_docs, min_count = 1, 
                               vector_size = 50, window = 5) # size changed to vector_size = 50
        vocabulary = model.wv.key_to_index # vocab changed to key_to_index
        # print(vocabulary)
        ...
        for sentence in final_corpus: # for each review/sentence
            ...
            for word in sentence.split(): # for each word in a review/sentence
                if word in glove_words:
                    vector += model.wv.get_vector(word) #  vector += model[word] replaced
                    cnt_words += 1
            if cnt_words != 0:
                vector /= cnt_words
